...
public class IrisVoxyRenderPipeline extends AbstractRenderPipeline {
    ...
    // Stamps depth=1.0 (far plane) into voxy's opaque depth buffer at every
    // pixel where voxy did NOT render (stencil == 0). Without this, the depth
    // buffer holds uninitialised/stale values at sky pixels, which fools any
    // downstream sampler (e.g. Photon's lod_depth_tex_solid) into thinking
    // sky pixels are LOD terrain — produces fringe artifacts at the LOD/sky
    // silhouette. Disabled when voxy.json sets skipShaderDepthHackFix=true.
    ...
    public IrisVoxyRenderPipeline(IrisVoxyRenderPipelineData data, AsyncNodeManager nodeManager, NodeCleaner nodeCleaner, HierarchicalOcclusionTraverser traversal) {
        super(nodeManager, nodeCleaner, traversal, data.shouldDeferTranslucency());
        this.data = data;
    ...
    protected void postOpaquePreTranslucent(Viewport<?> viewport) {
        // Stamp far-plane depth into voxy's opaque depth buffer at non-rendered
        // pixels (stencil == 0). Necessary so downstream samplers (shader-pack
        // composite passes that read vxDepthTexOpaque / lod_depth_tex_solid)
        // see a clean depth=1.0 at sky pixels rather than uninitialised values.
        // See field comment on shaderDepthHackFixTransformBlit for context.
        if (this.shaderDepthHackFixTransformBlit != null) {
            this.fb.bind();
            glEnable(GL_DEPTH_TEST);
            glColorMask(false, false, false, false);
            glDepthFunc(GL_ALWAYS);
            glStencilFunc(GL_EQUAL, 0, 0xFF);  // pass only where stencil == 0 (no voxy fragment)
            this.shaderDepthHackFixTransformBlit.blit();
            glStencilFunc(GL_EQUAL, 1, 0xFF);  // restore the normal voxy stencil mask
            glDepthFunc(GL_LEQUAL);            // upstream uses properties.closerEqualDepthCompare(); LEQUAL matches AbstractRenderPipeline.endRender()
