...
public class RenderResourceReuse {
    // One cached entry is enough to cover the give-back/get-or-create cycle of a
    // single render-system teardown+rebuild (iris reload, dimension change).
    // Anything beyond that is wasted VRAM since voxy only ever has one render
    // system alive per VoxyClientInstance. giveBack frees the surplus directly.
    private static final int MAX_CACHED = 1;
    ...
    public static void giveBackModelStoreTextureAtlas(GlTexture texture) {
        if (MODEL_TEXTURE_CACHE.size() >= MAX_CACHED) {
            texture.free();
        } else {
    ...
    public static void giveBackGeometryBuffer(GlBuffer geometryBuffer) {
        if (GEOMETRY_BUFFER_CACHE.size() >= MAX_CACHED) {
            geometryBuffer.free();
        } else {
