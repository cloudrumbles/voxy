...
import me.cortex.voxy.client.core.model.berbake.MultiCellFootprint;
...
public class RenderDataFactory {
    ...
    // Per-bucket growable scratch. Each of the 8 buckets (6 directional faces +
    // dual-sided + translucent) owns its own MemoryBuffer that doubles on
    // overflow. Previous design used a single 8 MiB buffer carved into 8 fixed
    // 1 MiB slots; overflowing a slot silently clobbered the next bucket and
    // emitted "Invalid size and/or offsets" GL errors. The fixed cap had to be
    // raised from 65 536 to 131 072 quads/face after aunaturel produced
    // ~66 000-quad sections — growable buffers remove the overflow hazard
    // entirely.
    //
    // Buckets retain their grown capacity across sections (only counters are
    // reset in generateMesh), so a thread that once meshed a large section
    // amortises future grows. Soft threshold below is informational.
    private static final int INITIAL_QUADS_PER_BUCKET = 1 << 13;//8 192 quads = 64 KiB starting per bucket
    private static final int LARGE_SECTION_WARN_THRESHOLD = 1 << 17;//Match the previous hard cap for parity with earlier telemetry
    private final MemoryBuffer[] quadBuckets = new MemoryBuffer[8];
    ...
    private final class Mesher extends ScanMesher2D {
        ...
        protected void emitQuad(int x, int z, int length, int width, long data) {
            ...
            // Sizes are 5-bit (1..32 stored as size-1): low 4 bits at the
            // historic field positions (length at 3-6 = size.x, width at
            // 7-10 = size.y), 5th bit in the quad word's spare bits (length
            // bit4 -> 42, width bit4 -> 43; the gap between state id and
            // biome id). Mirrored by quad_format.glsl extractSize.
            encodedPosition |= (((width - 1) & 0xF) << 7) | (((length - 1) & 0xF) << 3);
            ...
            int count = RenderDataFactory.this.quadCounters[bufferIdx];
            MemoryBuffer bucket = RenderDataFactory.this.quadBuckets[bufferIdx];
            if ((long) count * 8L >= bucket.size) {
                bucket = RenderDataFactory.this.growBucket(bufferIdx);
            }
            MemoryUtil.memPutLong(bucket.address + count*8L, quad);
            RenderDataFactory.this.quadCounters[bufferIdx] = count + 1;
    ...
    public RenderDataFactory(WorldEngine world, ModelFactory modelManager) {
        ...
        for (int i = 0; i < this.quadBuckets.length; i++) {
            this.quadBuckets[i] = new MemoryBuffer((long) INITIAL_QUADS_PER_BUCKET * 8L);
        }
    }

    // Doubles bucket capacity, copies live data across, frees old. Called from
    // emitQuad's hot path when a bucket fills. Returns the replacement buffer
    // so the caller can avoid re-loading from the field.
    private MemoryBuffer growBucket(int bucketIdx) {
        MemoryBuffer old = this.quadBuckets[bucketIdx];
        MemoryBuffer fresh = new MemoryBuffer(old.size * 2L);
        UnsafeUtil.memcpy(old.address, fresh.address, old.size);
        this.quadBuckets[bucketIdx] = fresh;
        old.free();
        return fresh;
    }
    ...
    private int prepareSectionData(final long[] rawSectionData) {
        ...
        for (int i = 0; i < 32*32*32;) {
            ...
            if (Mapper.isAir(block)) {//If it is air, just emit lighting
                ...
                int modelId = rawModelIds.get(Mapper.getBlockId(block));
                if (modelId < 0) {//Failed, so just return error (covers -1 and INFLIGHT_SENTINEL)
    ...
    private void acquireNeighborData(WorldSection section, int msk) {
        ...
        if ((msk&16)!=0) {//-z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*4] = raw[IntBitOps.expand(i,0b11111_00000_11111)|(0x1F<<5)];//pull the +z faces from the section
            }
            sec.release();
        }
        if ((msk&32)!=0) {//+z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*5] = raw[IntBitOps.expand(i,0b11111_00000_11111)];//pull the -z faces from the section
            }
            sec.release();
        }
    }
        ...
        for (int y = 0; y < 32; y++) {
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if (z == 30 && partialHasCount != 0) {//Hackfix for incremental count overflow issue
                    ...
                    //Clear the sum
                    sumA &= ~(IntBitOps.expand(Integer.toUnsignedLong(partialHasCount), X_I_MSK)*0x1F);
                    sumB &= ~(IntBitOps.expand(Integer.toUnsignedLong(partialHasCount)>>11, X_I_MSK)*0x1F);
                    sumC &= ~(IntBitOps.expand(Integer.toUnsignedLong(partialHasCount)>>22, X_I_MSK)*0x1F);
                }
                ...
                {//Dont need this as can just increment everything then -1 in mask
                    ...
                    sumA += IntBitOps.expand(imsk, X_I_MSK);
                    sumB += IntBitOps.expand(imsk>>11, X_I_MSK);
                    sumC += IntBitOps.expand(imsk>>22, X_I_MSK);
                }*/
    ...
    // Scratch buffer for multi-cell stamping. Lazily allocated; this factory is per
    // worker thread (one instance per mesh-gen worker), so a plain instance field is
    // already thread-local — no synchronization needed.
    private long[] multiCellScratch = null;

    // Multi-cell stamping (D1). Returns the section's raw voxel array, EXCEPT when the
    // section contains a registered multi-cell block: then it returns a COPY with synthetic
    // slice ids stamped into the overhang cells, leaving the section's real (disk-backed)
    // array untouched. prepareSectionData reads the returned array read-only.
    //
    // M3 lands this hook INERT: hasAnyRegistered() is false until a handler is registered
    // (M5), so the early-out is always taken and this is a single volatile read returning
    // the original array — provably zero behaviour change. The scan/copy/stamp body is
    // filled in M4/M5 once sliced models (and thus footprint bounds) exist.
    private long[] stampMultiCell(long[] raw) {
        if (!VoxyMultiCellRegistry.INSTANCE.hasAnyRegistered()) {
            return raw;//Common case (and all of M3): no multi-cell handlers -> no-op.
        }

        // Section index layout (WorldSection.getIndex): x in bits 0-4, z in 5-9, y in
        // 10-14. Only LOD0 (the first 32^3 entries) is stamped; the array is the full
        // mip pyramid but overhang only makes sense at the finest level.
        final int VOL = 32 * 32 * 32;

        // Pass 1 (gate): does any cell hold a registered multi-cell block whose footprint
        // is known (its centre has been baked)? Cheap: array read + isSynthetic skip +
        // registry lookup (cached) + footprint-store lookup. No copy yet.
        long[] copy = null;
        for (int i = 0; i < VOL; i++) {
            long cell = raw[i];
            int blockId = Mapper.getBlockId(cell);
            if (blockId == 0 || MultiCellSliceRegistry.isSynthetic(blockId)) {
                continue;//air, or an already-stamped synthetic cell (idempotent)
            }
            var state = this.world.getMapper().getBlockStateFromBlockId(blockId);
            var handler = VoxyMultiCellRegistry.INSTANCE.resolve(state);
            if (handler == null || !handler.shouldExpand(state)) {
                continue;
            }
            var box = MultiCellFootprintStore.INSTANCE.get(blockId);
            if (box == null) {
                continue;//centre not baked yet -> re-mesh once it is (footprint then known)
            }
            if (!box.overflowsCell()) {
                continue;//geometry fits its own cell -> nothing to expand
            }

            // Lazily copy on first real expansion, so sections with a registered block
            // but no actual overhang (or not-yet-baked) pay only the scan.
            if (copy == null) {
                if (this.multiCellScratch == null || this.multiCellScratch.length < raw.length) {
                    this.multiCellScratch = new long[raw.length];
                }
                copy = this.multiCellScratch;
                System.arraycopy(raw, 0, copy, 0, raw.length);
            }

            int bx = i & 0x1F, bz = (i >> 5) & 0x1F, by = (i >> 10) & 0x1F;
            int r = handler.maxFootprintRadius();
            for (var off : MultiCellFootprint.coveredCells(
                    box.minX(), box.minY(), box.minZ(), box.maxX(), box.maxY(), box.maxZ(), r)) {
                if (off.isOrigin()) {
                    continue;//the block's own cell already holds it
                }
                int nx = bx + off.dx(), ny = by + off.dy(), nz = bz + off.dz();
                if (nx < 0 || nx > 31 || ny < 0 || ny > 31 || nz < 0 || nz > 31) {
                    continue;//Decision B: clip at the section boundary
                }
                int ni = (ny << 10) | (nz << 5) | nx;
                long target = copy[ni];
                if (Mapper.getBlockId(target) != 0) {
                    continue;//Decision C: only stamp into AIR cells; never overwrite real geometry
                }
                int synthetic = MultiCellSliceRegistry.INSTANCE.getOrAllocate(blockId, off.dx(), off.dy(), off.dz());
                // Preserve the air cell's light/biome bits; replace only the block id.
                copy[ni] = Mapper.withBlockBiome(target, synthetic, Mapper.getBiomeId(target));
            }
        }
    ...
    public BuiltSection generateMesh(WorldSection section) {
        ...
        //Prepare everything (stampMultiCell returns the raw array unchanged unless a
        //registered multi-cell block is present — inert until a handler is registered)
        int neighborMskAndFlags = this.prepareSectionData(this.stampMultiCell(section._unsafeGetRawDataArray()));
        ...
        if (this.quadCount >= LARGE_SECTION_WARN_THRESHOLD) {
        ...
        for (int buffer = 0; buffer < 8; buffer++) {// translucent, double sided quads, 6 faces
            ...
            UnsafeUtil.memcpy(this.quadBuckets[buffer].address, ptr + coff*8L, (size* 8L));
            coff += size;
        }
    ...
    public void free() {
        for (MemoryBuffer bucket : this.quadBuckets) {
