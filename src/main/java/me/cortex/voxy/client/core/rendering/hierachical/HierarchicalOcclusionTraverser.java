...
public class HierarchicalOcclusionTraverser {
    ...
    // Most recent camera section coords (level-0 section units), captured each
    // frame in uploadUniform. Read on the same render thread by
    // forwardDownloadResult to sort incoming request batches near-to-far.
    private int cameraSecX;
    private int cameraSecY;
    private int cameraSecZ;

    // Reusable scratch for sortRequestsByDistance. Sized to the hard ceiling
    // on count (MAX_REQUEST_QUEUE_SIZE, set by the GPU traversal shader and
    // re-clamped in forwardDownloadResult). Only the render thread touches
    // these via the DownloadStream callback chain; no locking needed.
    private final long[] sortKeysScratch = new long[MAX_REQUEST_QUEUE_SIZE];
    private final long[] positionsScratch = new long[MAX_REQUEST_QUEUE_SIZE];
    ...
    private void uploadUniform(Viewport<?> viewport) {
        ...
        this.cameraSecX = viewport.section.x;
        this.cameraSecY = viewport.section.y;
        this.cameraSecZ = viewport.section.z;
        this.meshGen.setCameraSection(this.cameraSecX, this.cameraSecY, this.cameraSecZ);
        ...
        // Matches the trailing uint in traversal_dev.comp's SceneUniform.
        MemoryUtil.memPutInt(ptr, VoxyConfig.CONFIG.hideUnrefinedLodCells ? 1 : 0); ptr += 4;
    }
    ...
    private void forwardDownloadResult(long ptr, long size) {
        ...
        //}
        if (count != 0) {
            var batch = new MemoryBuffer(count*8L+8).cpyFrom(ptr-8);
            // Re-order requests near-to-far. The shader emits in BFS order
            // (outer LOD shell first), so the buffer is roughly back-to-front
            // by default. Sorting here flips it so finer meshing close to the
            // camera gets dispatched ahead of distant work.
            sortRequestsByDistance(batch.address + 8, count);
            this.nodeManager.submitRequestBatch(batch);
        }
    }

    private void sortRequestsByDistance(long entriesPtr, int count) {
        final int camX = this.cameraSecX;
        final int camY = this.cameraSecY;
        final int camZ = this.cameraSecZ;
        // Pack (distSq << 32) | originalIndex into one long per entry so the
        // sort is a primitive Arrays.sort with no boxing. Scratch arrays are
        // hoisted to fields sized at MAX_REQUEST_QUEUE_SIZE — count is
        // clamped to that ceiling upstream.
        final long[] sortKeys = this.sortKeysScratch;
        final long[] positions = this.positionsScratch;
        for (int i = 0; i < count; i++) {
            long pos = ((long) MemoryUtil.memGetInt(entriesPtr + i*8L)) << 32;
            pos |= Integer.toUnsignedLong(MemoryUtil.memGetInt(entriesPtr + i*8L + 4));
            positions[i] = pos;
            int level = WorldEngine.getLevel(pos);
            long l0x = (long) WorldEngine.getX(pos) << level;
            long l0y = (long) WorldEngine.getY(pos) << level;
            long l0z = (long) WorldEngine.getZ(pos) << level;
            long dx = l0x - camX;
            long dy = l0y - camY;
            long dz = l0z - camZ;
            long distSq = dx*dx + dy*dy + dz*dz;
            int distSqInt = (int) Math.min(distSq, Integer.MAX_VALUE);
            sortKeys[i] = ((long) distSqInt << 32) | Integer.toUnsignedLong(i);
        }
        java.util.Arrays.sort(sortKeys, 0, count);
        for (int i = 0; i < count; i++) {
            int origIdx = (int)(sortKeys[i] & 0xFFFFFFFFL);
            long pos = positions[origIdx];
            MemoryUtil.memPutInt(entriesPtr + i*8L, (int)(pos >>> 32));
            MemoryUtil.memPutInt(entriesPtr + i*8L + 4, (int) pos);
        }
    }
