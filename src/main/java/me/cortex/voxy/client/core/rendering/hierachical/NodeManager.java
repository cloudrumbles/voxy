...
public class NodeManager {
    ...
    // Top-level nodes with childExistence == 0 are intentionally supported: an
    // empty NodeChildRequest is created and sits in flight until processChildChange
    // adds children to it later (see makeLeafChildRequest at ~line 1150 and the
    // LEAF-with-request-in-flight path in processChildChange at ~line 329).
    // Non-top-level leaves with childExistence == 0 abort request creation with
    // a warn log (same handler).
    ...
    public void processRequest(long pos) {
        ...
        if (nodeType == NODE_TYPE_LEAF) {
            ...
            if (this.nodeData.isNodeRequestInFlight(nodeId)) {
                Logger.debug("Tried processing a node that already has a request in flight: " + nodeId + " pos: " + WorldEngine.pprintPos(pos) + " ignoring");
                return;
            }
