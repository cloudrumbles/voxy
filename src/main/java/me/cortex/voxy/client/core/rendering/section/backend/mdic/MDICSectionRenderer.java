...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    ...
    // Per-call working scratch for translucent depth-bucket counts + prefix sum.
    // cmdgen writes per-distance counts here, prefix-sum rewrites in place,
    // buildtranslucents reads the prefix sums and scatters sorted draw IDs into
    // viewport.drawCallBuffer before buildDrawCalls returns. The sorted result
    // lives in the per-viewport drawCallBuffer; this buffer is reset on entry
    // to the next buildDrawCalls call. Safe to share across viewports.
