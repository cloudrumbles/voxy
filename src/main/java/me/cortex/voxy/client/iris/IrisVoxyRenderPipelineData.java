...
public class IrisVoxyRenderPipelineData {
    ...
    private static int[] getDrawBuffers(int[] targets, ImmutableSet<Integer> stageWritesToAlt, RenderTargets rt) {
        ...
        int rtCount = rt.getRenderTargetCount();
        for(int i = 0; i < targets.length; i++) {
            if (targets[i] < 0 || targets[i] >= rtCount) {
                throw new ShaderLoadError("voxy.json declares draw-buffer index " + targets[i] +
                        ", but the iris pipeline only has " + rtCount +
                        " render targets (valid: 0.." + (rtCount - 1) + ")");
            }
