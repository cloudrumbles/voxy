...
public class WorldUpdater {
    ...
    private static long insertSectionLvlIntoWorld(VoxelizedSection section, WorldSection worldSection) {
        ...
        // The loop above wrote new values into worldSection.data; the
        // dataIsKnownAir invariant ("every long is AIR_FILL_VALUE") no longer
        // holds (the new values may or may not all be AIR_FILL_VALUE; we
        // don't check). Conservative clear so the array routes to the dirty
        // pool on release rather than corrupting the air pool's invariant.
        if (didStateChange) {
            worldSection.dataIsKnownAir = false;
        }
